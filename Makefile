...
SCRIPTS := $(shell find . -type f \( -name '*.lua' -or  -name '*.script' -or -name '*.render_script' \) -not -path '*lua_install*')
...
IOS_ICON_APP := resources/ios
ANDROID_ICON_APP := resources/android

ICONS := $(WIN32_ICON_APP) $(OSX_ICON_APP) $(LINUX_ICON_APP) $(IOS_ICON_APP) $(ANDROID_ICON_APP)

BOB_JAR := tmp/bob.jar
BOB_URL := http://d.defold.com/archive/683677d7d655e9c7aa3328ce9c72dd126391447d/bob/bob.jar
